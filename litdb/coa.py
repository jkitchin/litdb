...
def get_coa(orcid):
    ...
    while next_cursor:
        _filter = (f'author.orcid:https://orcid.org/{orcid}'
                   f',publication_year:>{four_years_ago - 1}')
        
        r = requests.get(url, params={'filter': _filter,
    ...
    # unique authors
    df = pd.DataFrame(uniq_authors, columns=['4', 'Name:',
    ...
    authors_df = pd.DataFrame(all_authors, columns=['4', 'Name:',
                                                'Organizational Affiliation',
                                                'Optional  (email, Department)',
                                                'Last Active',
                                                'OpenAlex pub id',

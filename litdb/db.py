...
def add_source(source, text, extra=None):
    ...
    c = db.execute('''insert or ignore into sources(source, text, extra, embedding, date_added) values (?, ?, ?, ?, ?)''',
                   (source, text, json.dumps(extra), embedding, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))

    # I think we should only do this if the previous line succeeded
    if c.rowcount > 0:
        db.execute('''insert into fulltext(source, text) values (?, ?)''', (source, text))
...
def update_filter(f, last_updated=None):
    ...
    db.execute('''update queries set last_updated = ? where filter = ?''',
